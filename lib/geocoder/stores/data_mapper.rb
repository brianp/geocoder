  ...
  module DataMapper
    ...
    def self.included(base)
      ...
      base.class_eval do
        ...
        def self.geocoded
          all(geocoder_options[:latitude].to_sym.not => nil,
            geocoder_options[:longitude].to_sym.not => nil)
        end

        # scope: not-geocoded objects
        def self.not_geocoded 
          all(geocoder_options[:latitude] => nil) |
            all(geocoder_options[:longitude] => nil)
        end
        ...
        #
        def self.within_bounding_box(bounds)
    ...
    module ClassMethods
      ...
      ##
      # TODO: Find a less lame way
      def using_sqlite?
        repository(:default).adapter.options['adapter'].match /sqlite/i
      end
